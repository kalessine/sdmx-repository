...
public class DataflowUtil {
    ...
    public static sdmx.repository.sdmx.entities.Dataflow createDatabaseDataflow(EntityManager em, Registry reg, DataflowType df) {
        sdmx.repository.sdmx.entities.DataStructureReference ref = DataStructureReferenceUtil.toDatabaseDataStructureReference(em, df.getStructure());
        em.persist(ref);
        sdmx.repository.sdmx.entities.Dataflow df1 = new sdmx.repository.sdmx.entities.Dataflow();
    ...
    public static Dataflow findDataflow(EntityManager em, String agency, String id, String version) {
        try {
            ...
            return (sdmx.repository.sdmx.entities.Dataflow) q.getSingleResult();
        } catch (Exception ex) {
            //ex.printStackTrace();
            return null;
        }
    ...
    public static List<sdmx.repository.sdmx.entities.Dataflow> searchDataflow(EntityManager em, String agency, String id, String version) {
        if ("*".equals(version) && "all".equals(id) && "all".equals(agency)) {
            Query q = em.createQuery("select d from Dataflow d");
            return (List<sdmx.repository.sdmx.entities.Dataflow>) q.getResultList();
    ...
    public static DataflowType toSDMXDataflow(sdmx.repository.sdmx.entities.Dataflow df) {
        if (df == null) {
            return null;
        }
